#!/usr/bin/env python3
"""
Analyze differences between 5'UTR and 3'UTR TE content.

Compares TE density, strand bias, and hit patterns between 5' and 3' UTR
regions for individual genes and across gene sets.
"""
...
from utils.paths import get_project_root, get_gene_lists_dir, get_results_dir
...
def main():
    ...
    gene_lists_dir = get_gene_lists_dir()

    # Load data
    print("Loading data...")
    utr3_top = load_te_data(results_dir / 'top_100_te_genes_FIXED.tsv')
    utr3_bottom = load_te_data(results_dir / 'bottom_100_te_genes_FIXED.tsv')
    utr3_all = {**utr3_top, **utr3_bottom}

    utr5_top = load_te_data(results_dir / '5utr_analysis/top_100_te_genes_FIXED.tsv')
    utr5_bottom = load_te_data(results_dir / '5utr_analysis/bottom_100_te_genes_FIXED.tsv')
    utr5_all = {**utr5_top, **utr5_bottom}

    utr3_strand = load_strand_bias(results_dir / 'strand_bias_by_utr.tsv')
    utr5_strand = load_strand_bias(results_dir / '5utr_analysis/strand_bias_by_utr.tsv')

    # Load germ plasm genes from source of truth file
    germ_plasm_genes = load_germ_plasm_genes(
        gene_lists_dir / 'germ_plasm_genes_consolidated.tsv'
    )
