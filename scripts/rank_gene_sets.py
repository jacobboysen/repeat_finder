#!/usr/bin/env python3
"""
Rank gene sets by genome-wide TE metrics.

Compares predefined gene sets (germ plasm, housekeeping, etc.) against
genome-wide TE density rankings and strand bias data.
"""
...
def main():
    ...
    strand_data = load_strand_bias_by_utr(results_dir / 'strand_bias_by_utr.tsv')

    # Load top/bottom 100 for density ranking
    top_100 = load_gene_te_data(results_dir / 'top_100_te_genes_FIXED.tsv')
    bottom_100 = load_gene_te_data(results_dir / 'bottom_100_te_genes_FIXED.tsv')
    ...
    # Symbol files
    symbol_files = {
        'germ_plasm': gene_lists_dir / 'germ_plasm_genes_consolidated.tsv',
        'housekeeping': gene_lists_dir / 'housekeeping_genes_consolidated.tsv',
        'somatic': gene_lists_dir / 'somatic_genes_consolidated.tsv',
        'cleared': gene_lists_dir / 'cleared_genes_consolidated.tsv',
        'adult': gene_lists_dir / 'adult_genes_consolidated.tsv',
    }
